...
def is_ignored_file(path: Path, root_path: Path) -> bool:
    ...
    """
    return path.stem.startswith(("_", ".")) or any(
        [p.stem.startswith(("_", ".")) for p in path.relative_to(root_path).parents]
    )
...
def get_files_in_path(
    path: Path, whitelist: set[Path] | None = None, suffix: str | None = None
) -> list[Path]:
    """
    ...
    return [
        f.relative_to(path)
        for f in files
        if (f.is_file() and not is_ignored_file(f, path) and suffix in (None, f.suffix))
    ]
...
def copy_file(input_path: Path, output_path: Path) -> None:
    ...
    assert input_path.is_absolute(), (
        f"input_path {str(input_path)} is relative. It must be absolute."
    )
    assert output_path.is_absolute(), (
        f"output_path {str(output_path)} is relative. It must be absolute."
    )
...
def convert_to_srgb(pil_image: ImageType) -> ImageType:
    ...
    if icc:
        ...
        try:
            pil_image = (
                ImageCms.profileToProfile(pil_image, src_profile, dst_profile)
                or pil_image
            )
        except ImageCms.PyCMSError:
            logging.warning(
                "Could not convert image color profile. Skipping conversion."
            )
    return pil_image
...
def get_image_transforms(original_path: Path) -> Iterator[dict]:
    ...
    for key, transform in config.image_transforms.items():
        ...
        transform_applies_to_file = any(
            original_path.match(pattern) for pattern in includes
            ...
            for suffix in output_suffixes:
                ...
                yield {
                    **transform,
                    ...
                    "output_mimetype": suffix_to_mimetype[
                        output_image_path.suffix.lower()
                    ],
                    "output_path": output_image_path.parent
                    / key
                    / output_image_path.name,  # It works if key is empty
                }


def make_picture_element(
    context: Context, entry_uri: EntryURI, img_attrs={}, a_attrs=None, sizes=None
) -> ElementTree.Element:
    """
    ...
    for mimetype, srcset_elements in sources_by_mimetype.items():
        source = ElementTree.SubElement(
            picture,
            "source",
            attrib={"type": mimetype, "srcset": ", ".join(srcset_elements)},
        )
        if sizes:
